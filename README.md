...
* Edit the script, `healthCheck.sh` and ensure the service name matches within the `setServiceName` variable. i.e., `ibkr-dashboard-1`
  * For example if the service name is "ibkr-dashboard2": `sed -i 's|ibkr-dashboard1|ibkr-dashboard2|g' /home/opc/repos/IBKR-Dashboard2/healthCheck.sh`
* Also ensure the `healthcheck.sh` `dashboardConName` variable matches the container name for nginx. i.e., `ibkr-dashboard-nginx2`
  * For example if the container name is `ibkr-dashboard-nginx2`: `sed -i 's|ibkr-dashboard-nginx|ibkr-dashboard-nginx2|g' /home/opc/repos/IBKR-Dashboard2/healthCheck.sh`
...
* Enter the crontab entry by executing `/home/opc/repos/IBKR-Dashboard1/add-healthCheck-crontab.sh` (or the correct path if the `IBKR-Dashboard1` portion is different)
